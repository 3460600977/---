...
<script>
  ...
  export default {
    ...
    watch: {
      recommendBuildings(val) {
        this.changePathPointType(-1, -2)
        val.forEach((item) => {
          let point = this.points[item]
          if (point) {
           if (this.judgePointType(point) &&point.type <0) {
             this.points[item].type = -1
           }
          } else {
            console.log('楼盘id不存在：'+ item)
          }
        })
...
<!--
  圆，多边形：<0 并且 != -3 并且 != 100 并且type = index 
      的点可以被选点操作选中和半径修改
  折线：因为是多段 所以要先把关于他的点type置成-1 
      然后 <0 并且 != -3 并且 != 100才能进行操作
 -->
