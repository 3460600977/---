      ...
      <div class="right-list">
        ...
        <div class="show_location_num">
          <el-button slot="reference" class="show-build-list">
            ...
            <div class="show-click" @click="dialogVisible = true"> 查看<i
          ...
          <el-dialog class="dialog-build-list"
            ...
            <div class="info-body">
              ...
              <div class="info-table">
                <el-table max-height="450" :data="resultData" v-loading="loading" class="info-el-table">
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                      <div v-if="col.prop === 'status'">
                        <span v-if="scope.row[scope.column.property].toString() === '1'"
                              class="normal info-status">正常</span>
                        <span v-else class="stop info-status">正在维护中</span>
                      </div>
                      <div v-else-if="col.prop === 'action'">
                        <a class="preview" href="#" @click="showPreviewPlayList(scope.row['deviceCode'])"
                           :deviceCode="scope.row['deviceCode']">预览</a>
                      </div>
                      <div v-else>
          ...
          <el-dialog
            ...
            width="30%"
            @open="getProjectPlayList">
            <PreviewBox v-if="playList.top && playList.bottom" :top="playList.top" :bottom="playList.bottom"></PreviewBox>
          </el-dialog>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      //预览展示
      showPreviewPlayList(deviceCode) {
          ...
          .then(res => {
            ...
            this.deviceInfo.time.forEach(item => {
              ...
              if (res.result[property] === '' || res.result[property] === null) {
                ...
                item.value = res.result['startTime'] + '~' + res.result['endTime']
              } else {
      ...
      getProjectPlayList() {
        ...
        this.$api.Report.getProjectPlayList(queryParam)
          .then(res => {
            ...
            this.playList.data.top.forEach(item => {
              type = this.playList.typeList[item.type - 1]
              topArr[i++] = {type: type, url: item.url}
            })
            i = 0;
            this.playList.data.down.forEach(item => {
              type = this.playList.typeList[item.type - 1]
              downArr[i++] = {type: type, url: item.url}
            })
            this.playList.top = topArr[0]
            this.playList.bottom = downArr[0]
            console.log(topArr, downArr)
