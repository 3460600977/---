<template>
  <div v-loading.fullscreen.lock="planData.loading" class="put-project">
    ...
    <PutMangeCard :title="'投放设置'" class="form-box">
      ...
      >
        ...
        <el-form-item class="mt-20" prop="projectType">
          <label slot="label">
            <span class="color-red">*</span>投放类型
          </label>
    ...
    <!-- 楼盘定向 -->
    <PutMangeCard style="margin-bottom: 20px" v-if="!isEdit" :title="'楼盘定向'" class="form-box">
      <el-tabs
        @tab-click="clearBuildDirection"
        class="thin-tab mt-15"
        v-model="buildingDirection.activeType"
      >
        <!-- 已有资源包 -->
      ...
      <div style="margin-top: 10px;" v-if="buildingDirection.activeType === 'exist'">
        <i class="color-red">*&nbsp;</i>可售投放数详见浮窗, 实际投放数据以支付成功为准。
      </div>
    </PutMangeCard>
      ...
      <template v-if="deviceNumber > 0">
        <div v-loading="buildingDirection.builds.loading">
          <ul class="msg-box color-text-1">
            <li class="item">
              <label class="name">楼盘数</label>
              <label class="bold">{{$tools.toThousands(buildsNumber, false)}}</label>个
            </li>
            <li class="item">
              <label class="name">单元数</label>
              <label class="bold">{{$tools.toThousands(unitNum, false)}}</label>个
            </li>
            <li class="item">
              <label class="name">点位数</label>
              <label class="bold">{{$tools.toThousands(deviceNumber, false)}}</label>个
            </li>
            <li class="item">
              <label class="name">覆盖人数</label>
              <label class="bold">{{$tools.toThousands(peopleNumber, false)}}</label>人
            </li>
          </ul>
          ...
          <ul class="money-box">
            <li class="item">
              ...
              <span
                class="color-red font-16 bold"
              >{{$tools.toThousands(buildingDirection.estimatePrice/100)}}</span>
            </li>
            <li class="item">
              <span>账户余额:&emsp;</span>
              <span>¥</span>
              ...
              <span
                class="color-red"
                v-if="buildingDirection.estimatePrice > userInfo.accountBalance"
              >(余额不足)</span>
            </li>
          </ul>
        </div>
      ...
      <div class="save-box">
        <div v-if="isEdit" class="float-right">
          <el-button
            ...
            :loading="formData.confirming"
            @click="confirmProject"
            style="width: 120px"
            type="primary"
          >
            <template v-if="formData.creativeStatus === 0 || formData.creativeStatus === 2">保存并关闭</template>
        ...
        <div v-else class="mid-between">
          ...
          <el-button
            :disabled="deviceNumber === 0 || !validataForm() || buildingDirection.estimatePrice > userInfo.accountBalance"
            :loading="formData.confirming"
            @click="confirmProject"
            style="width: 120px"
            type="primary"
    ...
    <el-dialog
      ...
      v-if="buildingDirection.activeType === 'exist'"
    >
      <map-choose-window
        ref="mapDialog"
...
export default {
  ...
  data() {
    let validateTime = (rule, value, callback) => {
      ...
      if (this.formData.projectType.value === 0 && this.formData.date) {
        ...
        if (
          ...
          callback(
            new Error(
              "按周投放的开始日期必须是周六, 结束日期必须为周五, 请正确输入!"
            )
          );
        }
      }
    ...
    return {
      ...
      formDataRules: {
        ...
        date: [{ validator: validateTime, trigger: "blur" }],
        count: [
  ...
  methods: {
    ...
    initProjectDetailById: async function() {
      ...
      this.$api.PutProject.GetProjectDetailById(
        +this.$route.query.editProjectId
      )
        .then(res => {
          ...
          this.formData = {
            ...
            name: resData.name,
            id: resData.id,
            industry: this.$tools.getObjectItemFromArray(
              this.industry.data,
              "industryId",
              resData.industry
            ),
            projectType: this.$tools.getObjectItemFromArray(
              projectConst.putType,
              "value",
              resData.projectType
            ), // 投放类型，0按周投放，1按天投放
            date: [resData.beginTime, resData.endTime],
            projectCity: resData.projectCity,
            deliveryMode: this.$tools.getObjectItemFromArray(
              projectConst.putWay,
              "value",
              resData.deliveryMode
            ), // 投放方式
            count: this.$tools.getObjectItemFromArray(
              projectConst.putFrequency,
              "value",
              resData.count
            ), // 投放频次
            second: this.$tools.getObjectItemFromArray(
              projectConst.putDuration,
              "value",
              resData.second
            ), // 投放时长
            type: this.$tools.getObjectItemFromArray(
              projectConst.screenType,
              "value",
              resData.type
    ...
    clearBuildDirection() {
      ...
      if (
        this.$refs.mapDialog &&
        this.buildingDirection.activeType === "exist"
      ) {
    ...
    getCityInsightDetail(id) {
      if (!id) return;
      this.$api.cityInsight.GetCityInsightDetailById(id).then(res => {
    ...
    changePageData() {
      ...
      if (!!this.buildingDirection.cityInsight.selectedItem) {
        this.getCityInsightDetail(
          this.buildingDirection.cityInsight.selectedItem.id
        );
      }
    ...
    uplaodBuild(event) {
      ...
      param = {
        beginTime:
          this.formData.projectType.value === 0
            ? this.formData.dateForWeekBegin
            : this.formData.dateForDay[0],
        endTime:
          this.formData.projectType.value === 0
            ? this.formData.dateForWeekEnd
            : this.formData.dateForDay[1],
        count: this.formData.count.value,
        deliveryMode: this.formData.deliveryMode.value,
        industry: this.formData.industry,
      ...
      this.$api.PutProject.DowloadBuildsTemplate()
        .then(res => {
          ...
          this.$tools.downLoadFileFlow(
            res,
            `导入楼盘数据模板${this.$tools.getFormatDate("YYmmdd")}.xls`
          );
        })
        .catch(res => {
          this.buildingDirection.templateFileDownloading = false;
    ...
    confirmProject() {
      ...
      if (this.isEdit) {
        this.$api.PutProject.EditProject({
          id: this.formData.id,
          name: this.formData.name
        })
          .then(res => {
             ...
             */
            if (
              this.formData.creativeStatus === 0 ||
              this.formData.creativeStatus === 2
            ) {
              this.$router.push({
                path: "/putManage",
                query: {
        ...
        disabledDate(date) {
          return (
            ...
            (now.getDate() + 1 == date.getDate() &&
              ...
              now.getHours() >= 21) ||
            // 按周投放
            (_this.formData.projectType.value == 0 &&
              date.getDay() != 5 &&
              date.getDay() != 6) ||
            (now.getDate() == date.getDate() &&
            now.getMonth() === date.getMonth() &&
            now.getFullYear() === date.getFullYear() && // 判断今天
